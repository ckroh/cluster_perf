...
def task_start_test(test_id, request_data):
	...
	elif 'node_type' in request_data:
		try:
			node_type=NodeType.objects.get(id=request_data['node_type'])
			cluster = node_type.cluster
		except:
			logging.error("REST API: startTest no node type defined for id %s" % request_data['node_type']) 
			return	
	...
	if 'hash' in request_data:
		...
		parameter = None
		for h in hash_list:
			test.startTest(cluster, test_config, nodes, h, parameter, node_type=node_type)
	else:
		...
		for n in sparameter:
			...
			test.startTest(cluster, test_config, nodes, hash, parameter, node_type=node_type)
			time.sleep(1)
	cluster.disconnect()
